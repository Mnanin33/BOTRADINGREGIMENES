...
def inicializar_base_datos():
    ...
    # Tabla de Usuarios (Soporta guardar API Keys asociadas)
    cursor.execute('''
...
def obtener_keys_usuario(user):
    """Recupera las llaves guardadas del usuario actual."""
    conn = conectar_db()
    cursor = conn.cursor()
...
def guardar_keys_usuario(user, api_key, secret_key):
    """Actualiza y guarda permanentemente las llaves de Binance del usuario correspondiente."""
    conn = conectar_db()
    cursor = conn.cursor()
...
if not st.session_state["autenticado"]:
    ...
    st.markdown(f"Bienvenido de vuelta, **{st.session_state['usuario_actual']}**. Esta versión incluye **Interés Compuesto**.")
    ...
    # Cargar automáticamente las llaves guardadas en la BD para el usuario autenticado
    ...
    if modo_operacion == "🟢 Trading Automático (En Vivo)":
        api_key = st.sidebar.text_input("API Key del Exchange", type="password", value=saved_api_key, placeholder="Introducir key...")
        secret_key = st.sidebar.text_input("Secret Key del Exchange", type="password", value=saved_secret_key, placeholder="Introducir secret...")
        
        if api_key != saved_api_key or secret_key != saved_secret_key:
            if st.sidebar.button("💾 Guardar Keys en mi Usuario"):
                guardar_keys_usuario(st.session_state["usuario_actual"], api_key, secret_key)
                st.sidebar.success("¡Llaves sincronizadas y guardadas con éxito!")
                time.sleep(1)
                st.rerun()
