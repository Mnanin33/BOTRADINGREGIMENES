...
def inicializar_base_datos():
    """Crea las tablas necesarias si no existen e incluye soporte para API Keys."""
    conn = conectar_db()
    cursor = conn.cursor()
...
# =====================================================================
# SISTERMA MULTIDIOMA (DICCIONARIO DE TRADUCCIÓN)
# =====================================================================
if "idioma" not in st.session_state:
    st.session_state["idioma"] = "Español 🇪🇸"

# Selector de idioma global en el tope (para persistencia del estado en login/main)
idiomas_disponibles = ["Español 🇪🇸", "English 🇬🇧"]

diccionario = {
    "Español 🇪🇸": {
        "acc_req": "🔐 Área Restringida - Introduce tus Credenciales",
        "user": "Usuario",
        "pass": "Contraseña",
    ...
    "English 🇬🇧": {
        "acc_req": "🔐 Restricted Area - Enter Your Credentials",
        "user": "Username",
        "pass": "Password",
        "btn_ingresar": "Log In",
        ...
        "param_opt": "🧠 Optimization Parameters",
...
# Shortcut variable based on choice
...
def render_login():
    ...
    # Selector de idioma disponible en la ventana de Login
    ...
    with st.form("formulario_login"):
        usuario = st.text_input(current_lan["user"])
        password = st.text_input(current_lan["pass"], type="password")
...
if not st.session_state["autenticado"]:
    ...
    # =====================================================================
    # 2. SELECCIÓN DE MODO DE OPERACIÓN COMERCIAL
    # =====================================================================
    st.sidebar.markdown("---")
    ...
    api_key = ""
    secret_key = ""
    if modo_operacion == lan["modos"][2]: # Trading Automático (En Vivo)
    ...
    if df.empty:
        ...
        # =====================================================================
        # 10. PROTOCOLO DE CONEXIÓN Y ALMACENAMIENTO
        # =====================================================================
        st.markdown("---")
        st.subheader(lan["gestion_con"])
        
        if modo_operacion == lan["modos"][0]: # Desactivado
            st.warning(lan["warn_apagado"])
            
        elif modo_operacion == lan["modos"][1]: # Solo Señales (Manual)
            st.info(lan["info_manual"].format(orden=lado_orden.upper()))
            
        elif modo_operacion == lan["modos"][2]: # Trading Automático (En Vivo)
